-- Adapted from Adam Vandervorst/PathMap lean/Main.lean @ f477a91 (MIT, see LICENSE-UPSTREAM).
import PathMapsSpec
...
One-shot:
    ...
    pathmaps-oracle <input-file>       -- decode and run the file's bytes
    pathmaps-oracle                    -- read the bytes from stdin
    pathmaps-oracle --act <input-file> -- ArenaCompactTree mode: skip the
                                      -- operations an ACT read source cannot
                                      -- serve, matching differential/src/bin/act_trace.rs

Resident:

    pathmaps-oracle --server [--act]   -- one process, many inputs
