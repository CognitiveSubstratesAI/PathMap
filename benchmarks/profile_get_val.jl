...
let i = 0
    for c in 1:200, _ in 1:200
        ...
        PM.set_val_at!(
            M,
            vcat(Vector{UInt8}("cat" * lpad(string(c), 3, '0') * ":"), CS[rand(1:36, 8)]),
            Int32(i)
        )
    end
end
const HIT = let kk = vcat(Vector{UInt8}("cat100:"), CS[rand(1:36, 8)])
    PM.set_val_at!(M, kk, Int32(-1))
    kk
end
...
let (ci, rt) = code_typed(PM.get_val_at, (typeof(M), Vector{UInt8}); optimize=true)[1]
    for (i, st) in enumerate(ci.code)
        ...
        (
            ...
            occursin("view", s) || occursin("indexed_iterate", s)
        ) && println("  [", i, "] ", first(s, 140))
    end
