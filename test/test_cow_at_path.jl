...
function _shared_pair()
    s = PathMaps.PathMap{Int}()
    PathMaps.set_val_at!(s, _b("x:1"), 1)
    PathMaps.set_val_at!(s, _b("x:2"), 2)
...
_snapshot(m) = (PathMaps.val_count(m),
    PathMaps.get_val_at(m, _b("x:1")),
    PathMaps.get_val_at(m, _b("x:2")),
    PathMaps.get_val_at(m, _b("x:3")))
...
@testset "writes through write_zipper_at_path do not corrupt a sharing map" begin
    ...
    @testset "remove_val_at! — the shape that surfaced it" begin
        (s, t) = _shared_pair()
        before = _snapshot(s)
        PathMaps.remove_val_at!(t, _b("g:x:1"), false)
        @test PathMaps.get_val_at(t, _b("g:x:1")) === nothing   # the target really changed
    ...
    @testset "the MORK read/write isolation shape (Space.jl:1749)" begin
        ...
        btm = PathMaps.PathMap{Int}()
        PathMaps.set_val_at!(btm, _b("a:1"), 1)
        PathMaps.set_val_at!(btm, _b("a:2"), 2)
        PathMaps.set_val_at!(btm, _b("b:1"), 3)
        read_btm = PathMaps.PathMap{Int, PathMaps.GlobalAlloc}(
            copy(btm.root), btm.root_val, btm.alloc
        )
        before = PathMaps.val_count(read_btm)

        PathMaps.remove_val_at!(btm, _b("a:1"), false)
        @test PathMaps.get_val_at(btm, _b("a:1")) === nothing
        @test PathMaps.get_val_at(read_btm, _b("a:1")) == 1
        @test PathMaps.val_count(read_btm) == before
    end
end
