# test_trie_ref_cow.jl — `make_map` must SHARE a subtrie without letting writes leak between maps.
#
# THE DEFECT THIS PINS (found 2026-08-05, latent — the function had no callers). `make_map` did:
...
@testset "make_map — shares structurally, isolates writes (COW)" begin
    ...
    @testset "the derived map sees the source's contents" begin
        src = mk()
        m = P.make_map(P.trie_ref_at_path(src, UInt8[]))
    ...
    @testset "removal is isolated too, in both directions" begin
        ...
        src2 = mk()
        m2 = P.make_map(P.trie_ref_at_path(src2, UInt8[]))
    ...
    @testset "a subtrie taken at a NON-EMPTY path isolates as well" begin
        ...
        m = P.make_map(P.trie_ref_at_path(src, Vector{UInt8}("pre:")))
    ...
    @testset "a MID-LINE prefix extracts the region (get_node_at_key, not node_get_child)" begin
        # SECOND defect in _tr_get_focus_rc, fixed 2026-08-05. Upstream (trie_ref.rs:306) resolves a
        ...
        # BOUNDARY — so a mid-line prefix returned `nothing` and make_map produced an EMPTY map,
        # silently and with no error.
        #
        # MEASURED before the fix, on this exact trie:
        #     trie_ref_at_path(m, "pre:")  -> make_map == []          (!!)   mid-line
        #     trie_ref_at_path(m, "pre:a") -> make_map == [a, b, c]          at a boundary
        ...
        keys_of(x) = (z=read_zipper(x); o=String[];
            while to_next_val!(z)
                push!(o, String(copy(path(z))))
            end; sort(o))

        @test keys_of(P.make_map(P.trie_ref_at_path(m, Vector{UInt8}("pre:")))) ==
            ["aa", "ab", "ac"]
        @test keys_of(P.make_map(P.trie_ref_at_path(m, Vector{UInt8}("pre:a")))) ==
            ["a", "b", "c"]
        @test keys_of(P.make_map(P.trie_ref_at_path(m, Vector{UInt8}("other:")))) ==
            ["zz"]
        @test keys_of(P.make_map(P.trie_ref_at_path(m, UInt8[]))) ==
            ["other:zz", "pre:aa", "pre:ab", "pre:ac"]
        # a prefix matching NOTHING must give an empty map, not a wrong one
        @test isempty(keys_of(P.make_map(P.trie_ref_at_path(m, Vector{UInt8}("nope:")))))
        # and the mid-line region stays isolated, same as the boundary case
        r = P.make_map(P.trie_ref_at_path(m, Vector{UInt8}("pre:")))
