...
@testset "ADR-001 increment 1 — slab scaffold" begin
    ...
    @testset "slab store/load round-trip + growth" begin
        ...
        o1 = PathMap.slab_reserve!(s, 4)
        PathMap.slab_store!(s, o1, Int32(-42))
        ...
        @test PathMap.slab_load(s, o2, SH) ==
            PathMap.SlabHandle(UInt32(99), PathMap.TAG_BRIDGE)
        for i in 1:60
            o = PathMap.slab_reserve!(s, 4)
            PathMap.slab_store!(s, o, Int32(i))
        end
    ...
    @testset "step 2: DenseByteNode in-slab round-trip" begin
        ...
        entries = [
            DBN(PathMap.SlabHandle(UInt32(100), PathMap.TAG_LINELIST), Int32(7), true),
        ...
        for _ in 1:30
            PathMap.dbn_pack!(s, mask, entries)
        end   # force regrow
    ...
    @testset "step 3 (slab read lookup): byte→entry matches packing" begin
        ...
        DBN = PathMap.DBNEntry{Int32}
        entries = [
            DBN(PathMap.SlabHandle(UInt32(100), PathMap.TAG_LINELIST), Int32(7), true),  # 'A'
            DBN(PathMap.SLAB_NIL, Int32(0), false)]                                       # 'z'
        h = PathMap.dbn_pack!(s, mask, entries)
        iA = PathMap.dbn_slab_index(s, h, 0x41)
        iz = PathMap.dbn_slab_index(s, h, 0x7a)
        ...
        @test eA.child == PathMap.SlabHandle(UInt32(100), PathMap.TAG_LINELIST) &&
            eA.val == Int32(7)
