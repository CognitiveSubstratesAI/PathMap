...
#
# It was `test_join_consumes_source.jl` (commit `8fe6319`), pinning "`join_map_into!` CONSUMES its
...
@testset "a join never writes into its source operand" begin
    ...
    @testset "the target is still correct — the fix changes only whose node gets written" begin
        a = _mk(A_MIN)
        PathMaps.join_map_into!(PathMaps.write_zipper(a), _mk(S_MIN))
        @test _paths(a) == ["a:a", "aa", "ab", "ab:", "b", "b:a"]
        @test PathMaps.val_count(a) == 6
    end

    @testset "join_map_into! — source survives, handed DIRECTLY" begin
        a = _mk(A_MIN)
        s = _mk(S_MIN)
        PathMaps.join_map_into!(PathMaps.write_zipper(a), s)
        @test _paths(s) == S_MIN            # `s` used to gain "ab:" here
        @test PathMaps.val_count(s) == 3
    end

    @testset "join_map_into! — source survives THROUGH A SHARING CLONE" begin
        ...
        shared = _mk(S_MIN)
        PathMaps.join_map_into!(PathMaps.write_zipper(_mk(A_MIN)), _share(shared))
    ...
    @testset "fuzz 00324 verbatim: RESET · ASCEND 4 · RESET · JOINMAP" begin
        ...
        PathMaps.reset!(wz)
        PathMaps.ascend!(wz, 4)
        PathMaps.reset!(wz)
        PathMaps.join_map_into!(wz, _share(shared))
        @test _paths(shared) == S_MIN       # used to become [a:a, aa, aab, ab, ab:]
        @test PathMaps.val_count(shared) == 3
        @test PathMaps.val_count(a) == 8     # and the join itself is unaffected
    end

    @testset "join_into! — the read-zipper form had the SAME hole, via the same node path" begin
        # Not a bonus check: `join_into!` reaches `pjoin_dyn` identically, so it corrupted the
        ...
        shared = _mk(S_MIN)
        PathMaps.join_into!(PathMaps.write_zipper(_mk(A_MIN)), _anr(_share(shared)))
        @test _paths(shared) == S_MIN
    end
    ...
    @testset "the sibling consuming ops, same two handover modes" begin
        ...
        for (name, op) in (
            ("graft_map", (wz, s) -> PathMaps.graft_map!(wz, s)),
            ("meet_into", (wz, s) -> PathMaps.meet_into!(wz, _anr(s), false, s.root_val)),
            (
                "meet_into prune",
                (wz, s) -> PathMaps.meet_into!(wz, _anr(s), true, s.root_val)
            ),
            (
                "subtract_into",
                (wz, s) -> PathMaps.subtract_into!(wz, _anr(s), false, s.root_val)
            ),
            ("subtract_into prune",
                (wz, s) -> PathMaps.subtract_into!(wz, _anr(s), true, s.root_val)),
            ("restrict", (wz, s) -> PathMaps.restrict!(wz, _anr(s)))
        )
            @testset "$name" begin
                direct = _mk(S_MIN)
    ...
    @testset "repeated joins from one source — the source is reusable, not single-shot" begin
        ...
        b = _mk(A_MIN)
        PathMaps.join_map_into!(PathMaps.write_zipper(a), s)
        PathMaps.join_map_into!(PathMaps.write_zipper(b), s)
