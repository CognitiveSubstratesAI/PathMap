# test_shared_node_id.jl — `zipper_shared_node_id` / `zipper_is_shared` and the cached catamorphism.
#
...
@testset "shared_node_id guard (upstream zipper.rs:2638-2653)" begin
    @testset "a shared subtrie with different focus values at its two paths" begin
        ...
        for valued in ("a", "b")                      # the value on the first- or the later-visited path
            t = PMS{UInt64}()
            for p in ("a", "b")
                wz_graft_map!(write_zipper_at_path(t, cus(p)), s)
            end
            set_val_at!(t, cus(valued), UInt64(9))
            @test count_side(t) == 5
            @test count_cached(t) == 5
            @test tree_cached(t) == tree_side(t)
            z = read_zipper(t); zipper_descend_to!(z, cus(valued))
            @test zipper_val(z) == 9
            @test zipper_shared_node_id(z) === nothing  # valued focus: never an id
        end
    end
    ...
    @testset "unshared and dangling foci have no id" begin
        ...
        z = read_zipper(m); zipper_descend_to!(z, cus("ba"))
        @test zipper_path_exists(z)
        @test !zipper_is_shared(z)
        @test zipper_shared_node_id(z) === nothing
        z = read_zipper(m); zipper_descend_to!(z, cus("a"))
        @test !zipper_is_shared(z)                     # refcount 1
        @test zipper_shared_node_id(z) === nothing
    end
    ...
    @testset "a shared, value-free node has the same id on both paths" begin
        ...
        za = read_zipper(t); zipper_descend_to!(za, cus("a"))
        zb = read_zipper(t); zipper_descend_to!(zb, cus("b"))
        @test zipper_is_shared(za) && zipper_is_shared(zb)
        @test zipper_shared_node_id(za) !== nothing
        @test zipper_shared_node_id(za) == zipper_shared_node_id(zb)
    ...
    @testset "cata_test_cached (upstream morphisms.rs:1922)" begin
        ...
        for _level in 1:3
            next = PMS{UInt8}()
            for ii in 0:2
                wz_graft_map!(write_zipper_at_path(next, UInt8[ii]), m)
            end
            m = next
        end
