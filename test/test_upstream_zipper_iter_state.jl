...
# descends and sibling moves, count-returning ascends.
...
# Phase 3 landed: these tests now call the real 0.4.0 API (`path`, `depth`, `ascend!` → Int,
# `descend_first_byte!` → Union{Nothing,UInt8}, …); the temporary Shim040 block is gone.
...
create_path_at!(m, p) = create_path!(write_zipper_at_path(m, collect(UInt8, p)))

function value_iteration_history_test_map()
    m = PM{UInt64}()
    set_val!(write_zipper(m), UInt64(0))
...
@testset "read_zipper_to_next_val_after_descend_first_byte_matches_descend_to" begin
    ...
    @test path_of(a) == path_of(d); @test to_next_val!(d)
    @test to_next_val!(a); @test path_of(a) == path_of(d); @test val(a) == val(d)
...
@testset "read_zipper_to_next_val_after_each_ascent_matches_direct_position_in_multi_node_map" begin
    ...
    for (label, up) in ascents
        ...
        up(z)
        @test path_of(z) == prefix
        d = read_zipper(m); descend_to!(d, prefix)
        @test to_next_val!(d)
        @test to_next_val!(z)
        @test path_of(z) == path_of(d)
        @test val(z) == val(d)
    end
end
...
@testset "read_zipper_to_next_sibling_from_partial_focus_matches_direct_focus" begin
    ...
    @test to_next_sibling_byte!(z) == UInt8('w'); @test path_of(z) == b("w")
    @test to_next_sibling_byte!(z) === nothing; @test path_of(z) == b("w")
    d = read_zipper(m); descend_to!(d, b("w")); @test to_next_val!(d)
    @test to_next_val!(z); @test path_of(z) == path_of(d); @test val(z) == val(d)
end
...
@testset "read_zipper_to_next_k_path_after_descend_first_k_path_matches_descend_to" begin
    ...
    d = read_zipper(m); descend_to!(d, path_of(a))
    @test path_of(a) == path_of(d); @test to_next_k_path!(d, 2)
    @test to_next_k_path!(a, 2); @test path_of(a) == path_of(d)
end
...
@testset "read_zipper_to_next_k_path_after_k_path_descent_and_ascent_matches_unmoved_focus" begin
    ...
    @test path_of(a) == path_of(u); @test to_next_k_path!(u, 2)
    @test to_next_k_path!(a, 2); @test path_of(a) == path_of(u)
end
...
@testset "read_zipper_descend_first_byte_after_failed_descend_first_k_path" begin
    ...
    @test !descend_first_k_path!(z, 3); @test path_of(z) == UInt8[]
...
@testset "read_zipper_to_next_step_after_exhausted_to_next_k_path" begin
    ...
    @test to_next_val!(z)
    @test !to_next_k_path!(z, 5); @test path_of(z) == UInt8[0, 0, 0, 1, 0, 2, 2, 2]
    @test to_next_step!(z); @test path_of(z) == UInt8[0, 0, 0, 1, 0, 2, 2, 2, 1]
end

@testset "read_zipper_descend_first_byte_after_exhausted_k_path_and_ascent" begin
    z = read_zipper(unit_map([b("ab"), b("wxyz")]))
    descend_to!(z, b("w"))
    @test descend_first_k_path!(z, 1); @test path_of(z) == b("wx")
    @test !to_next_k_path!(z, 1); @test path_of(z) == b("w")
    @test ascend_byte!(z); @test path_of(z) == UInt8[]
    @test descend_first_byte!(z) == UInt8('a')
end
...
@testset "read_zipper_to_next_val_after_exhausted_k_path" begin
    ...
    @test at_root(z); @test to_next_val!(z); @test path_of(z) == b("ab")
    ...
    @test !to_next_k_path!(z, 1); @test path_of(z) == UInt8[2]
    @test to_next_val!(z); @test path_of(z) == UInt8[2, 2, 1, 2, 3, 2, 3]
end
...
@testset "read_zipper_to_next_sibling_after_missing_path_below_leaf" begin
    ...
    @test descend_first_byte!(z) === nothing
    @test ascend_byte!(z); @test path_of(z) == UInt8[10]
...
@testset "read_zipper_missing_dense_path_uses_lower_bound_for_iteration" begin
    ...
    @test !path_exists(s)
    @test to_next_sibling_byte!(s) == UInt8('z'); @test path_of(s) == b("z")
    v = read_zipper(m); move_to_path!(v, b("mm"))
    @test !path_exists(v)
    @test to_next_sibling_byte!(v) === nothing
    @test to_next_val!(v); @test path_of(v) == b("z")
end
...
@testset "read_zipper_native_and_default_value_iteration_can_be_interleaved" begin
    ...
    walk(step) = (z = read_zipper(m); out = Tuple{Vector{UInt8}, UInt64}[];
        i = 0; while step(z, i); push!(out, (path_of(z), val(z))); i += 1; end; out)
...
@testset "read_zipper_to_next_val_after_every_movement" begin
    ...
    for (label, f) in ways
        @testset "$label" begin
            z = read_zipper(m)
            f(z)
            @test path_of(z) == UInt8[0]
            @test to_next_val!(z); @test path_of(z) == UInt8[0, 0]; @test val(z) == 7
            @test to_next_val!(z); @test path_of(z) == UInt8[1]
            @test !to_next_val!(z)
        end
    end
    ...
    for (label, up) in ups
        @testset "$label" begin
            z = read_zipper(two)
            @test to_next_val!(z); @test to_next_val!(z); @test path_of(z) == UInt8[3, 3]
            up(z)
            @test at_root(z)
    ...
    seen = [path_of(z)]
    while to_next_k_path!(z, 2); push!(seen, path_of(z)); length(seen) > 8 && break; end
    @test seen == [UInt8[0, 0], UInt8[0, 1], UInt8[1, 0], UInt8[1, 1]]
end

@testset "read_zipper_fork_root_to_next_step_terminates" begin
    m = u64_map([UInt8[0, 0, 2, 3, 0] => 1]); create_path_at!(m, UInt8[0, 3])
...
@testset "read_zipper_empty_node_below_dangling_path_ascend" begin
    m = u64_map([UInt8[0, 0, 2] => 1]); create_path_at!(m, UInt8[0, 3])
    z = rz_at(m, UInt8[0, 3])
...
@testset "read_zipper_missing_focus_between_key0_and_descendant_key1" begin
    ...
    @test to_next_sibling_byte!(z) == UInt8('b'); @test path_of(z) == b("ab")
    z = read_zipper(m); descend_to!(z, b("aaa"))
    @test to_next_sibling_byte!(z) === nothing; @test path_of(z) == b("aaa")
    z = read_zipper(m); descend_to!(z, b("aa"))
    @test to_next_k_path!(z, 1); @test path_of(z) == b("ab")
    z = read_zipper(m); descend_to!(z, b("aaa"))
    @test !to_next_k_path!(z, 1); @test path_of(z) == b("aa")
end

@testset "read_zipper_k_path_base_mismatch_exit" begin
    m = unit_map([UInt8[0, 0, 0, 0, 0, 0, 1], UInt8[2]])
    z = read_zipper(m)
    @test to_next_val!(z); @test !to_next_k_path!(z, 2); @test path_of(z) == UInt8[0, 0, 0, 0, 0]
    @test descend_first_byte!(z) == 0x00
    z = read_zipper(m)
    @test to_next_val!(z); @test !to_next_k_path!(z, 2)
    @test to_next_val!(z); @test path_of(z) == UInt8[0, 0, 0, 0, 0, 0, 1]
end
...
@testset "read_zipper_rooted_to_next_val_exhaustion" begin
    ...
    z = rz_at(m, UInt8[1, 2])
    @test to_next_val!(z); @test !to_next_val!(z)
    @test to_next_step!(z); @test path_of(z) == UInt8[3]
end
