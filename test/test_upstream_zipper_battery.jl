...
# `no method matching reset!(::ReadZipperUntracked)`.
...
mask_bytes(z) = sort!(collect(child_mask(z)))
pathstr(z) = String(copy(collect(path(z))))
...
function run_battery(kind::String)
    @testset "upstream zipper conformance battery — $kind" begin
        ...
        @testset "zipper_moving_basic_test (upstream zipper.rs:3230)" begin
            ...
            descend_to!(z, b"m")
            descend_to!(z, b"'")
            @test path_exists(z)                       # focus = rom'  (' is the lowest byte)
            ...
            @test to_next_sibling_byte!(z) === nothing            # only 3 children: ' a u
            ...
            @test mask_bytes(z) == [UInt8('i')]
            @test ascend!(z, 1) == 1
            @test descend_indexed_byte!(z, 1) !== nothing
            @test mask_bytes(z) == [UInt8('n')]
            @test ascend!(z, 1) == 1
            @test descend_indexed_byte!(z, 2) !== nothing
        ...
        @testset "zipper_with_root_path (upstream zipper.rs:3277)" begin
            ...
            # Test `ascend`
            descend_to!(z, b"manus")
            @test pathstr(z) == "manus"
            @test ascend!(z, 1) == 1
            @test pathstr(z) == "manu"
            @test ascend!(z, 5) == 4       # 5 > 4 remaining bytes: ascends what it can, landing at root
            @test pathstr(z) == ""
            @test at_root(z)
            descend_to!(z, b"mane")
            @test pathstr(z) == "mane"
        ...
        @testset "descend_to_existing_test1 (upstream zipper.rs:3744)" begin
            ...
            @test descend_to_existing!(z, b"bowling") == 3
            @test pathstr(z) == "bow"
            reset!(z)

            @test descend_to_existing!(z, b"can") == 3
            @test pathstr(z) == "can"
            reset!(z)

            @test descend_to_existing!(z, UInt8[]) == 0
            @test pathstr(z) == ""
            reset!(z)
        end
        ...
        @testset "descend_to_existing_test2 (upstream zipper.rs:3762)" begin
            ...
            @test descend_to_existing!(z, Vector{UInt8}("arrow" * zeros160)) == 5
            @test pathstr(z) == "arrow"
            reset!(z)

            @test descend_to_existing!(z, Vector{UInt8}("arr" * zeros160)) == 3
            @test pathstr(z) == "arr"
        end
        ...
        @testset "descend_to_existing_test3 (upstream zipper.rs:3775)" begin
            ...
            descend_to!(z, b"arrow00000")
            @test path_exists(z) == false
            @test pathstr(z) == "arrow00000"

            @test descend_to_existing!(z, b"0000") == 0
            @test pathstr(z) == "arrow00000"
        end
        ...
        @testset "zipper_indexed_bytes_test1 (upstream zipper.rs:3345)" begin
            ...
            reset!(z)
            @test descend_indexed_byte!(z, 7) === nothing
            @test is_val(z) == false
            @test child_count(z) == 7
            @test pathstr(z) == ""
            ...
            descend_to!(z, b"000")
            @test val(z) === UNIT_VAL              # upstream: Some(&())
            @test pathstr(z) == "000"
            @test child_count(z) == 0
            @test descend_indexed_byte!(z, 1) === nothing
            @test pathstr(z) == "000"
            ...
            @test descend_indexed_byte!(z, 1) !== nothing
            @test pathstr(z) == "1"
            @test val(z) === nothing               # upstream: None
            @test child_count(z) == 1
            @test descend_indexed_byte!(z, 1) === nothing
            @test val(z) === nothing
            @test pathstr(z) == "1"
        ...
        @testset "indexed_zipper_movement1 (upstream zipper.rs:3635)" begin
            ...
            function descend_byte(zz, byte::UInt8)
                for i in 0:(child_count(zz) - 1)   # upstream i is 0-based; ours takes the same index
                    @test descend_indexed_byte!(zz, i) !== nothing
                    if last(path(zz)) == byte
                        break
                    else
                        @test ascend!(zz, 1) == 1
                    end
            ...
            @test pathstr(z) == ""
            @test child_count(z) == 4
            descend_byte(z, UInt8('r'))
            @test pathstr(z) == "r"
            @test child_count(z) == 2
            @test descend_until!(z) == false
            descend_byte(z, UInt8('o'))
            @test pathstr(z) == "ro"
            @test child_count(z) == 1
            @test descend_until!(z) == true
            @test pathstr(z) == "rom"
            @test child_count(z) == 3
        ...
        @testset "zipper_descend_until_max_bytes_test1 (upstream zipper.rs:3444)" begin
            ...
            descend_to!(z, b"a0")
            @test pathstr(z) == "a0"
            @test descend_until_max_bytes!(z, 2)
            @test pathstr(z) == "a0ab"

            reset!(z)
            descend_to!(z, b"a1")
            @test pathstr(z) == "a1"
            @test descend_until_max_bytes!(z, 3)
            @test pathstr(z) == "a1mno"

            reset!(z)
            descend_to!(z, b"a0")
            @test pathstr(z) == "a0"
            @test descend_until_max_bytes!(z, 10)
            @test pathstr(z) == "a0abc"

            reset!(z)
            descend_to!(z, b"a0")
            @test pathstr(z) == "a0"
            @test !descend_until_max_bytes!(z, 0)
            @test pathstr(z) == "a0"
        end
        ...
        @testset "zipper_ascend_until_test3 (upstream zipper.rs:3504)" begin
            ...
            descend_to!(z, b"12345")
            @test path_exists(z)
            @test pathstr(z) == "12345"
            @test ascend_until_branch!(z) > 0
            @test pathstr(z) == ""
            @test at_root(z)
            ...
            descend_to!(z2, b"12345")
            @test path_exists(z2)
            @test pathstr(z2) == "12345"
            @test ascend_until!(z2) > 0
            @test pathstr(z2) == "1234"          # "1234" is a branch only
            @test is_val(z2) == false
            @test child_count(z2) == 2
            @test ascend_until!(z2) > 0
            @test pathstr(z2) == "123"           # "123" is a value only
            @test child_count(z2) == 1
            @test is_val(z2) == true
            @test ascend_until!(z2) > 0       # Jump over "12" because it's neither a branch nor a value
        ...
        @testset "zipper_ascend_until_test4 (upstream zipper.rs:3578)" begin
            ...
            @test child_count(z) == 1
            @test is_val(z) == true
            @test ascend_until!(z) > 0        # Jump over "12" because it's neither a branch nor a value
        ...
        @testset "zipper_ascend_until_test5 (upstream zipper.rs:3617)" begin
            ...
            descend_to!(z, Vector{UInt8}(long_a * "B"))
            @test path_exists(z) == false
            @test ascend_until!(z) > 0
        ...
        # WITHOUT MOVING the zipper. Our Zipper.jl has `val` (focus only) and PathMap has
        ...
        @testset "zipper_value_locations (upstream zipper.rs:3677)" begin
            ...
            @test path_exists(z)
            @test is_val(z) == false
            descend_to!(z, b"mulus")
            @test is_val(z) == true

            reset!(z)
            descend_to!(z, b"roman")
            @test path_exists(z)
            @test is_val(z) == true          # "roman" is a value AND an interior branch
            descend_to!(z, b"e")
            @test is_val(z) == true
            @test ascend!(z, 1) == 1
            descend_to!(z, b"u")
            @test is_val(z) == false         # "romanu" is a dangling prefix of "romanus"
            descend_until!(z)
            @test is_val(z) == true
        end
        ...
        @testset "zipper_byte_iter_test2 (upstream zipper.rs:3832)" begin
            ...
            @test descend_first_byte!(z) !== nothing
            @test collect(UInt8, path(z)) == UInt8[1]
            @test to_next_sibling_byte!(z) === nothing
            @test collect(UInt8, path(z)) == UInt8[1]
        end
        ...
        @testset "zipper_byte_iter_test3 (upstream zipper.rs:3842)" begin
            ...
            @test collect(UInt8, path(z)) == UInt8[]
            @test descend_first_byte!(z) !== nothing
            @test collect(UInt8, path(z)) == UInt8[6]
            @test descend_first_byte!(z) !== nothing
            @test collect(UInt8, path(z)) == UInt8[6, 193]
            @test descend_first_byte!(z) !== nothing
            @test collect(UInt8, path(z)) == UInt8[6, 193, 7]
        end
        ...
        @testset "zipper_byte_iter_test4 (upstream zipper.rs:3854)" begin
            ...
            for _ in 1:64
                descend_first_byte!(z) === nothing && break
            end
            ...
            @test path_exists(z)
            @test pathstr(z) == "ABC"
            @test descend_indexed_byte!(z, 1) !== nothing     # 'D' is index 0, 'd' is index 1
            @test pathstr(z) == "ABCd"
            @test descend_first_byte!(z) !== nothing
        ...
        @testset "zipper_byte_iter_test5 (upstream zipper.rs:3880)" begin
            ...
            @test to_next_sibling_byte!(z) !== nothing
            reset!(z)
            descend_to!(z, UInt8[2, 197, 97, 120, 105])
            @test to_next_sibling_byte!(z) !== nothing
        end
        ...
        @testset "zipper_iter_test1 (upstream zipper.rs:4063)" begin
            ...
            keys = ["arrow", "bow", "cannon", "rom'i", "roman", "romane", "romanus",
                "romulus",
        ...
        @testset "zipper_iter_test2 (upstream zipper.rs:4084)" begin
            ...
            while to_next_val!(z)
                @test is_val(z) == true
                @test collect(UInt8, path(z)) == be8(count)
                count += 1
        ...
        # ── zipper_val_at_test (upstream zipper.rs:3903) — was UNPORTABLE until val_at existed
        @testset "zipper_val_at_test (upstream zipper.rs:3903)" begin
            _, z = battery_zipper(["arrow", "bow", "cannon", "roman", "romane", "romanus",
                "romulus", "rubens", "ruber", "rubicon", "rubicundus", "rom'i"])
            @test val_at(z, b"") === nothing
            @test val_at(z, b"roman") === UNIT_VAL
            @test val_at(z, b"romane") === UNIT_VAL
            @test val_at(z, b"roma") === nothing
            @test val_at(z, b"romanu") === nothing
            @test val_at(z, b"ruber") === UNIT_VAL
            @test val_at(z, b"rub") === nothing
            @test val_at(z, b"zzz") === nothing

            descend_to!(z, b"ro")
            @test path_exists(z)
            @test val_at(z, b"") === nothing
            @test val_at(z, b"m") === nothing
            @test val_at(z, b"man") === UNIT_VAL
            @test val_at(z, b"mane") === UNIT_VAL
            @test val_at(z, b"manus") === UNIT_VAL
            @test val_at(z, b"manu") === nothing
            @test val_at(z, b"mulus") === UNIT_VAL
            @test val_at(z, b"mulu") === nothing
            @test val_at(z, b"zz") === nothing

            descend_to!(z, b"man")
            @test path_exists(z)
            @test val_at(z, b"") === UNIT_VAL
            @test val_at(z, b"e") === UNIT_VAL
            @test val_at(z, b"us") === UNIT_VAL
            @test val_at(z, b"u") === nothing
            @test val_at(z, b"zz") === nothing

            reset!(z)
            descend_to!(z, b"romanx")
            @test !path_exists(z)
            @test val_at(z, b"") === nothing
            @test val_at(z, b"e") === nothing
        end
        ...
        @testset "zipper_val_at_long_path_test (upstream zipper.rs:3947)" begin
            ...
            m = PM{UnitVal}()
            set_val_at!(m, long_key, UNIT_VAL)
            z = BATTERY_MAKE_Z[](m, UInt8[])
            @test val_at(z, long_key) === UNIT_VAL

            descend_to!(z, b"rom")
            @test path_exists(z)
            @test val_at(z, rel) === UNIT_VAL
            @test val_at(z, almost) === nothing
            @test val_at(z, b"zzz") === nothing
        end
...
# and it is the layer carrying the open factor_count / last-factor-guard risk recorded in
# test/differential/ADAPTATIONS.md entry 2 — `secondary` is a LIVE STACK under upstream PR #56's
# invariant, so factor_count sums a static count with a moving depth.
#
